...
class Ubicacion_fisicas_model extends CI_Model {
    ...
    public function select($modo, $select = array(), $condicion = array(), $order = '') {
        ...
        $sql = "SELECT " . $campos . " FROM UBICACION_FISICAS WHERE 1 = 1 " . $where . " " . $order;
        //echo $sql;exit;
    ...
    public function filas(){
        $sql = "select count(ID) FILAS from UBICACION_FISICAS";
        $query = $this->db->query($sql);
        $row = $query->row_array();
        return $row['FILAS'];
    }
